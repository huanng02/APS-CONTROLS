...
namespace QuanLyGiuXe.Services.Connection
{
    ...
    public class CameraConnectionManager
    {
        ...
        private void DisposeConnection(string urlKey, CameraConnection conn)
        {
            ...
            if (conn.CaptureTask != null)
            {
                try
                {
                    conn.CaptureTask.Wait(1000);
                }
                catch {}
            }
        ...
        private async Task CaptureLoopAsync(CameraConnection conn, int targetWidth, int targetHeight, int maxFps, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        conn.Capture = new VideoCapture(conn.Url, VideoCaptureAPIs.FFMPEG);
                    }
                    else if (int.TryParse(conn.Url, out int index))
                    {
                        conn.Capture = new VideoCapture(index, VideoCaptureAPIs.DSHOW);
                    }
                    else
                    {
                        int foundIndex = -1;
                        ...
                        if (foundIndex >= 0)
                        {
                            conn.Capture = new VideoCapture(foundIndex, VideoCaptureAPIs.DSHOW);
                        }
                        else
                        {
                            conn.Capture = new VideoCapture(conn.Url);
                        }
                    }

                    if (conn.Capture == null || !conn.Capture.IsOpened())
                      {
                          conn.Capture?.Dispose();
                      ...
                      conn.Capture.Set((VideoCaptureProperties)38, 1);
                      ...
                      using (conn.Capture)
                      {
                          ...
                          while (!token.IsCancellationRequested)
                          {
                              ...
                              try
                              {
                                  readSuccess = conn.Capture.Read(tempMat);
                              }
                              catch
                              {
                                  readSuccess = false;
